...
import '../viewmodel/tindak_lanjut_viewmodel.dart';

/// Layar dosen untuk menindaklanjuti izin yang sudah approved wali.
/// Setiap izin punya banyak jadwal terdampak — dosen tandai status per jadwal
/// (izin / sakit / alpha) untuk jadwal yang dia ampu.
class TindakLanjutIzinScreen extends StatefulWidget {
  final User user;
  const TindakLanjutIzinScreen({super.key, required this.user});
...
class _TindakLanjutIzinScreenState extends State<TindakLanjutIzinScreen> {
  ...
  Future<void> _tandai(Map<String, dynamic> izin, Map<String, dynamic> tindak) async {
    ...
    if (ok) await _vm.load(widget.user);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Tindak Lanjut Izin'),
      ),
          ...
          return ValueListenableBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (_, list, __) {
                ...
                return Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Tidak ada izin untuk ditindaklanjuti.',
              ...
              return RefreshIndicator(
                onRefresh: () => _vm.load(widget.user),
  ...
  Widget _buildIzinCard(Map<String, dynamic> izin) {
    final namaMhs = izin['namaMahasiswa']?.toString() ?? izin['mahasiswaId']?.toString() ?? '-';
    ...
    // Filter hanya jadwal yang dosen ini yang ampu
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(namaMhs, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
            Text('NIM: $mahasiswaId', style: const TextStyle(fontSize: 12, color: AppColors.grayMedium)),
            const SizedBox(height: 8),
            ...
            const Text('Jadwal yang Anda ampu:',
  ...
  Widget _tindakTile(Map<String, dynamic> izin, Map<String, dynamic> tindak) {
    ...
    String label;
    switch (statusFinal) {
      case 'izin': color = Colors.blue; label = 'IZIN'; break;
      case 'sakit': color = Colors.red; label = 'SAKIT'; break;
      case 'alpha': color = Colors.orange; label = 'ALPHA'; break;
      default: color = Colors.grey; label = 'BELUM';
    }
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(tindak['namaMK']?.toString() ?? tindak['jadwalId']?.toString() ?? '-',
                    style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                Text('${tindak['jamMulai'] ?? ''} - ${tindak['jamSelesai'] ?? ''}',
